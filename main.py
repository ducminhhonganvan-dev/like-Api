...
api_keys = {
    "quametlon": {"exp": "30/07/2095", "remain": 999, "max_remain": 999, "last_reset": None}
}
...
async def get_token(acc, session):
    ...
    try:
        ...
        async with session.get(url) as response:
            ...
            print(f"[get_token] RESPONSE (preview): {text[:200]}")
...
async def refresh_tokens():
    global tokens
    try:
        accounts = get_next_accounts(300)
        if accounts:
        ...
        with tokens_lock:
            tokens = {}
    # schedule next run
    threading.Timer(1200, lambda: asyncio.run(refresh_tokens())).start()

async def clean_and_replace_tokens():
    global tokens
    tokens_to_remove = []
    with tokens_lock:
        tokens_to_remove = [token for token, count in tokens.items() if count >= 66]
...
async def sendLikes(uid):
    ...
    with uids_lock:
        if uid in used_uids and used_uids[uid] == today:
            return {"Failed": " like Của Acc Này Max Rồi"}, 200
    ...
    if len(token_list) < 200:
        needed_tokens = 250 - len(token_list)
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        return {
            "result": {
                "User Info": {
                    "Tài khoản UID": InfoBefore["1"]["1"],
                    "Tài khoản Name": InfoBefore["1"]["3"],
                    "Tài khoản Region": InfoBefore["1"]["5"],
                    "Tài khoản Level": InfoBefore["1"]["6"],
                    "Tài khoản Likes": InfoBefore["1"]["21"]
                },
                "Likes Info": {
                    "Likes Sau": LikesBefore,
                    "Likes Trước": LikesBefore + LikesAdded,
                    "Likes Đã Thêm": LikesAdded,
                    "Likes start of day": max(0, LikesBefore + LikesAdded - 200),
                },
                "API": {
...
def FF_LIKES():
    ...
    if is_key_valid(key) is None:
        return jsonify({"message": "key not found, To buy key contact for zalo 0356758265"}), 200
    if not uid:
        return 'UID missing!'
    try:
        uid = str(uid).strip()
